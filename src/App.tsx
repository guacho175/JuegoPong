    ...
    const loadRanking = async () => {
      ...
      if (sheetUrl) {
        try {
          ...
          const res = await fetch(`${sheetUrl}?juego=pong`, { cache: 'no-store' });
          if (res.ok) {
      ...
      const saved = JSON.parse(localStorage.getItem('neon_pong_ranking') || '[]');
      setRanking(saved);
      setIsLoadingRanking(false);
    };
      ...
      <div className="flex-1 flex flex-col w-full z-10 relative min-h-0">
        ...
        <main className="flex-1 flex flex-col items-center justify-center w-full h-full p-1 sm:p-2 lg:p-4 overflow-hidden relative bg-[radial-gradient(#1e1e24_1px,transparent_1px)] [background-size:40px_40px]">
          <motion.div
            ...
            className="w-full h-full flex items-center justify-center pointer-events-auto max-w-4xl max-h-[800px]"
          >
            <GameBoard
              onGameOver={() => {
